...
|---|---|---|
| **v1 audience scope** | **Local player only** — *superseded 2026-08-24, see M9* | Remote detection, crowd throttling and per-player identity were deferred to v2. The audience filter existed as a *seam* from day one, and M9 replaced the seam with the real classifier. **Self-only remains the shipped default**, so the decision still governs what a fresh install does. |
...
| M6 | Settings, throttle, gates | ✅ **done, 2026-08-17.** The status line here previously read "not started — the biggest functional gap", which was **wrong**: `Gates` (cutscene / zoning / PvP / gpose / quest / territory) and `Throttle` (2 s per-caster cooldown, auto-attack skip, casts-only, muted actions) were already written and wired into `OnCast`. What was genuinely missing and is now built: **config migration and repair** (version ladder, null-collection and NaN guards, range clamps — a null collection used to throw from inside the action hook), a **way to mute an individual action** (`MutedActionIds` was checked by the throttle and cleared by the UI, but nothing could ever add to it), **settings consolidated** onto the Settings tab from the diagnostics tab, with `UseVoiceSliderNotSe`, `Enabled` and `FallBackToTestTone` exposed for the first time, **debounced persistence** of learned actions (previously lost on a crash, and overwritten by any mid-session save), and a **live** gate-status readout instead of one cached from the last cast. |
| M7 | Native sink | ✅ **reworked to native-only + pre-compiled packs, verified in game (2026-08-18).** The M7.0 build (2026-08-17) proved the pipeline: `ScdForge` + `NativeVoiceSink` + `CompositeVoiceSink` behind `IVoiceSink`, encoding off the game thread, warm-at-registration. But native playback was *opportunistic* — every refusal (first-play encode, warm-up, every fresh pitch roll) silently fell through to NAudio, so "native mode" mostly sounded like NAudio. The rework inverts it: **`SinkMode { Auto, NativeOnly, ManagedOnly, Off }`** replaces `PreferNativeSink` (config v2 migration; `true → NativeOnly`); in NativeOnly a refusal is a counted `DropStage.SinkRefused` with a reason string, never a fallback. **`PackBuilder`** compiles every mapped variant ahead of time (Apply button + 2 s debounced auto-apply) and **warms per job** via the shared `JobIndex` (moved out of `MainWindow`), since warmed handles are unevictable client memory; a cold clip that is nonetheless cast self-heals by warming on the spot. **Pitch rides on `PlaySound`'s `speed` argument** (`VoiceRequest.Speed`; `NativePitchViaSpeed`, default on, ⚠ unverified — checklist item (f)), collapsing random pitch to one variant per clip; off, rolls are baked at half-semitone steps. Also landed: per-caller Penumbra tags (`Warcry.Clips`/`Spike`/`Probe` — the shared tag let the spike silently wipe the forge's redirects), cached `PenumbraAvailable`, latched template-load failure, non-zero Penumbra codes rolled back, `ScdWriter.TryParse` no longer throws on hostile offsets, `MaxConcurrent` enforced once over both sinks, scheduler propagates delayed refusals, `VariantKey` pinned to invariant culture (was culture-sensitive — "1,0000" on this French client), audition pinned to the managed sink. **Sound pack tab** shows plan/compile/warm state and hosts the §6 (b)–(e)+(f) checklist — **all items passed in game 2026-08-18**, including (f), so `NativePitchViaSpeed` stays on and the default sink mode moved to Auto. Remaining: which slider (Voice vs SE) governs the bus was not noted, and the `SoundDataRefCount` climb has not been re-measured under `autoRelease: true`. |
| M7.1 | Caster-following voices | 🟡 **built 2026-08-24, ⚠ unverified in game.** A position handed to `PlaySound` is fixed for the whole clip and the listener follows the camera, so any displacement skill left its own voiceline behind and it faded out mid-word. Fixed by `VoicePositionMode { Follow (default), Fixed, Listener }`: `Follow` passes `autoRelease: false`, retains the `SoundData*` and pushes the caster's live position every frame from `NativeVoiceSink.Update`; `Fixed`/`Listener` keep the exact call verified on 2026-08-18, so both fallbacks are old code paths rather than new ones. New `Game/CasterLocator` resolves **by entity id every frame** (`SearchByEntityId`, *not* `SearchById`) with a `LocalPlayer` fast path — never a cached object, since an actor can despawn mid-line. **All three modes now read the position at dispatch**, which independently fixes a bug nobody had noticed: the scheduler holds a cast line by its ~0.45 s `CastRemaining`, so every cast line was playing at where the character stood half a second earlier. Teardown: `StopAll()` on `IVoiceSink` (also stops NAudio through a loading screen, which it previously did not), called on `TerritoryChanged` and before the redirects drop in `Dispose`. Leak instruments on the Status tab — `following` must return to 0 after every line; `orphaned` must stay 0 or follow mode's ownership assumption is false. See §9 for what still needs the game. **First in-game attempt did not follow at all**, and one real defect came out of it: `SetPosition` was gated on the same `Started` latch that guards *retirement*, so a voice whose `IsPlaying()` never reported true was never positioned — a line that plays perfectly at its start position, with every counter reading healthy. Position is state on a slot we own, not an event; the two decisions are now separate. Added `moves`/`startsSeen` counters plus a first-move position read-back, because "we never called it" and "the engine ignored it" sound identical from the speakers. **Second attempt then proved `SoundData::SetPosition` alone is inaudible** — ~280 updates over 11–15 y of movement, reading back exactly what was written, line still fixed — so a third attempt pushed to `SoundController::SetPosition` one level down in the audio driver — which **stopped every voice after one frame** with the unmapped `Vector4.W` at 0, and **works correctly with W = 1**. ✅ **Confirmed following in game 2026-08-24** by toggling the push live: with it the line tracks the caster through a displacement skill, without it it does not. Following therefore takes two writes per frame, and only the driver one is audible — see §9, including the W trap and the kill check that keeps a future patch from turning it back into a silent session. Everything else from the exercise stands on its own: position read at dispatch rather than snapshot, `StopAll` teardown, and the pool instruments. Three debugging notes that each cost a round — **Dalamud's file sink drops everything below Information**, so a Debug diagnostic is one nobody will ever read; never gate positioning on `IsPlaying()`; and `GetFileName()` reports the Penumbra-resolved local path, not the game path. Still open: (a) `autoRelease`/`ReleaseSoundData` ownership (no orphans seen so far) and (c) the `SoundDataRefCount` climb. |
| M8 | Release + repo | ❌ not started; csproj still carries `TODO-your-name` |
| M9 | v2: other players | 🟡 **built 2026-08-24, ⚠ unverified in game.** `Gating/AudienceFilter` replaces the `if (!ev.IsLocalPlayer) return;` seam in `Plugin.OnCast`. Buckets are a **set, not a first-match winner**: a friend in your party is `Party \| Friend`, so "play lines for friends" cannot silently skip them — the narrowest *enabled* member is the cooldown tier, and the full membership is what a profile's target is matched against. Every classification input (`HomeWorld`, `CurrentDistance`, `IsPartyMember`/`IsAllianceMember`/`IsFriend`, name) is read off `Character*` **inside the detour** into a `Game/CasterFacts`, so the filter never touches a game object and never allocates — names arrive as an FNV-1a hash (`Game/PlayerId`, ASCII-folded) because `NameString` allocates per cast. `Gating/CrowdWatch` scans the object table at 1 Hz through the *same* `CasterFacts` shape, so a player counted toward the crowd is exactly a player who would be admitted. Throttle stages 3 (crowd scaling) and 4 (token bucket, self bypasses, new `DropStage.RateLimited`) landed with it, plus a max-distance gate. **Profiles gained a target** (`ProfileMatch.Audience` + `Names`), so different people can get different clips; specificity is banded so who **outranks** what-they-sound-like (names 128, tier ×32, appearance ≤18) and pre-existing profiles default to `Anyone`, i.e. "everyone", with no migration step. New **People tab**; config v5. **The load-bearing change is warm scoping**: `PackBuilder` now warms the union of jobs from `CrowdWatch` rather than one job, because warmed handles are unevictable — still an earned, bounded set, never "every job in the game". Not verified in game: whether remote `CasterKey` reads correctly off another player's `DrawData`, and what the resident cost actually is with `Other` enabled in a hub city. |
| M10 | Grunt suppression | 🟡 **built 2026-08-25, ⚠ unverified in game.** `Audio/GruntSuppressor` — the second `SoundManager.PlaySound` hook §9 item 3 asked about, now answered "in v1". One setting, `GruntMode { Off, WhenVoiced, Always }`, **defaulting to `Off` and never migrated off it** — a dropdown rather than two checkboxes because the two suppressing members are alternatives at different widths, not a feature and its modifier, and only `WhenVoiced` needs a window or a caster attributed to it. The spike's `SoundManagerWatcher` was the seed — its 18-argument detour, its no-allocation UTF-8 path filter and its fault breaker all survive; the 120-row ring buffer did not. **Only the attack banks**: `soundNumber is not 1 and not 2`, because 1 is damage-taken and 2 is death (the group table parsed in `docs/native-spike.md`), so a suppressed character still grunts when hurt and killed — the same split `ScdForge` honours when it retargets audio indices. **Attribution is positional**: emitter world coordinates within 1.5 y of an armed caster, which works only because the spike measured `distance=0.00` and settled that `PlaySound` takes world, not listener-relative, coordinates. Windows are armed at **snapshot**, in `OnCast` beside `Throttle.Mark`, and their positions refreshed each frame through the shared `CasterLocator`; the detour never reads the object table. **Suppression is by gain, never refusal** — `Original` is always called, with the volume argument zeroed and `result->Volume` zeroed after — because the game passes `autoRelease: false` for its own grunt and retains the pointer. The hook is installed disabled and enabled only while a setting is on and `Enabled` is true, since `PlaySound` fires for every sound in the client. Not verified in game: **that `volume: 0f` actually silences the call** (fallback lever: `SoundVolumeCategory.NoPlay`), that 1.5 y separates two casters in practice, and that damage/death grunts survive a live window. |

**⚠ The paragraph that used to sit here was stale and wrong**, and it was believed twice before anyone checked the code. It claimed there was "no throttle of any kind" and "no PvP, cutscene, gpose or territory gates" long after all of them had been written and wired. Status lines in this document are prose, not build output — **read the code before trusting a ❌ here.**
...
| — | Suppressing the game's **own** battle grunt | 🟡 | **Built 2026-08-25, unverified in game** | `Audio/GruntSuppressor` hooks `SoundManager.PlaySound` and zeroes the volume for `vo_battle` paths — attack banks only, attributed to an armed caster by world position. The Penumbra alternative (redirect the `Vo_Battle_PC_*` paths to a silent SCD) was not needed and stays unbuilt: it cannot be made conditional, because the resource is loaded and cached once. See M10. |
...
> **Built 2026-08-24 as `Gating/AudienceFilter`, and it diverges from the sketch below in one
> way that matters.** The plan said "first match wins". It does not: `MembershipOf` returns
> **every** bucket the caster belongs to, so a friend in your party is `Party | Friend`.
> First-match-wins would classify them `Party` alone, and "play lines for friends" would then
> silently skip them — the single most confusing thing this filter could do. What the narrowest
> *enabled* bucket is still decides the cooldown tier and what the Events tab shows; the full
> membership set is what a profile's target is matched against.
>
> The other differences: the classifier reads a `Game/CasterFacts` rather than a `CastEvent`,
> so the 1 Hz `Gating/CrowdWatch` scan classifies through exactly the same code; names are
> compared as FNV-1a hashes of the raw UTF-8 (`Game/PlayerId`), never as strings, because this
> runs in the detour; and the distance gate is evaluated **before** the tier, since a party
> member across the zone is still nobody you can hear and refusing early spends no sound-pool
> slot. `Whitelist` shipped as `Named`, and the blocklist as `Configuration.BlockedPeople`.
>
> **`EnabledBuckets` shipped as `Configuration.Audience`, still defaulting to `Self`** — the v5
> migration forces it for existing configs, because an update that starts voicing strangers
...
**Implemented 2026-08-24 as `VoicePositionMode.Follow` (M7.1) — and there is no actor-attach API to prefer instead.** Re-checked against the shipped `FFXIVClientStructs.dll`: all 18 `PlaySound` parameters are scalars, no `SoundData` field references a `GameObject`, and `SoundDataSoundController` exposes only `SetPosition(Vector4*)` / `SetIsNonPositional(bool)` with no owner. The game must push a position per frame for its own actor sounds too, so following *is* the mechanism, not a workaround for the lack of one.

One fact this section did not have, and it is what makes a retained handle defensible: **the pool is a fixed 256-entry array**, so a stale `SoundData*` is never a *dangling* pointer — reading through it cannot fault, it can only describe somebody else's sound. `NativeVoiceSink` therefore identifies a retained slot before touching it, by the audio index plus the file name *as the engine itself reported it at play time* (calibrated, not assumed — if the engine stores a different string, comparing against ours every frame would call every voice an orphan and leak every slot). The test deliberately ignores `IsActive`: a false negative drops a voice without releasing it, which is the leak it exists to prevent, and `IsActive` is a guess during resource loading and after playback ends — the two states it is asked about most.

**MEASURED 2026-08-24 — `SoundData::SetPosition` does not move a voice that is already sounding.** This is the load-bearing discovery of M7.1 and it is not written down anywhere public. Two displacement casts, instrumented: ~280 position updates each, over **11.24 y** and **14.78 y** of genuine caster movement, `positional=true`, and the slot reading back *exactly* the coordinates written — and the line audibly stayed where it started, both times. So the virtual stores the value in the `SoundData` record and the mixer never re-reads it; the driver takes its own copy when the voice starts.

**SOLVED 2026-08-24 — the live path is `SQEX.CDev.Engine.Sd.Driver.SoundController::SetPosition(Vector4*)`, and `Vector4.W` must be 1.** Reached via `SoundData::GetSoundController()` (non-null for our voices; a signature-located member function, `E8 ?? ?? ?? ?? E9 ?? ?? ?? ?? 41 8B BE`, resolving to `7FF77ACE0F90` on 7.55). Confirmed in game by toggling it live: with the push, a line tracks the caster through a displacement skill; without it, it does not.

**The W component is the whole trap, and it is not cosmetic.** Passed as **0**, this call *stops the voice*: every line died one frame — 5 ms — after its first push, `started=true` then gone, where the same clip without the push played its full 1.37 s. The audible result was total silence, not a mispositioned line, which reads like a crash-free catastrophe rather than a bad argument. Passed as **1** — the homogeneous-coordinate reading — it follows correctly. **Do not "tidy" it to 0.**

So following takes **two writes per frame**, and only the second is audible: `SoundData::SetPosition` to keep the record and its `IsPositional` coherent for everything that reads them, then the driver call. Do not simplify the second away; several in-game rounds went into establishing that the first alone does nothing.

Three rules that fell out of this and outlive it. **Guard `SoundController.Addresses.SetPosition.Value != 0`** before calling anything signature-located from a per-frame loop: on a patch where the signature stops matching it is a call through null, i.e. an access violation that takes the client with it. **Keep the kill check** in `NativeVoiceSink.Retire` — a retained voice that ends within `DriverKillSeconds` of a push latches following off for the session, so if a patch makes this call destructive again the cost is one clipped line instead of a silent session. And **count driver pushes separately from record writes** (`moves` vs `driverMoves` on the Status tab), because the difference between those two numbers is exactly the difference between following and only appearing to.

`Listener` (non-positional) remains a legitimate choice rather than a fallback — no falloff, no retained handle — for anyone who would rather their own voice were not in the world at all.

⚠ **Still only the game can settle these.** (a) `autoRelease: false` really keeps the slot ours and `ReleaseSoundData` is the sufficient counterpart — a nonzero `orphaned` on the Status tab falsifies it; nothing has been orphaned so far. (b) **Falsified, see above** — and whether the driver-level push works is the open question in its place. (c) The `SoundDataRefCount` climb, already flagged unmeasured under `autoRelease: true` in the M7 row, is now load-bearing. (d) A position pushed while `IsLoadingSoundResource` is not silently dropped. **If (a) fails**, the fallback is `autoRelease: true` plus the same identity check — viable *because* the pool is a fixed array — at the cost of a one-frame window where a recycled slot could be nudged. **If the driver push also fails**, following is not reachable through this API and `Listener` becomes the recommended answer for displacement skills.

Two side notes worth keeping. `IsPlaying()` and `Started` are for deciding when to *retire* a voice and must never gate positioning — an earlier build coupled them, and a voice whose `IsPlaying()` never reported true was simply never positioned, which reads exactly like the engine ignoring the API. And the engine's `GetFileName()` reports the **Penumbra-resolved local file** (`C:/…/.cache/scd/<hash>.scd`), not the game path requested, so a retained-slot identity check must snapshot what the engine says rather than assert equality with what we asked for.
...
#### Suppressing the game's own battle grunt (built 2026-08-25, off by default)

When our voiceline plays, the game's native grunt for the same action doubles up. `Audio/GruntSuppressor` hooks `SoundManager.PlaySound` and zeroes the gain for `vo_battle` paths. One setting, `Audio/GruntMode { Off, WhenVoiced, Always }`, default `Off`.

Four decisions worth keeping:

- **Attack banks only.** `soundNumber is not 1 and not 2` — group 1 is damage taken, group 2 is death (the table parsed from a real container in `docs/native-spike.md`). A blanket path match would take a character's whole voice, not their action grunt.
- **The window opens at snapshot**, not when our line plays. The grunt is animation-driven at 5–1071 ms after snapshot, fixed per action, so it routinely arrives *before* a cast line that is still waiting out its cast bar. Default 1.5 s, `GruntWindowSeconds`, and only `WhenVoiced` reads it.
- **Attribution by world position**, ±1.5 y against a per-frame-refreshed caster position. Only viable because the spike measured `distance=0.00` on a real row and settled that `PlaySound` takes world coordinates.
- **Zero the gain, never refuse the call.** The game passes `autoRelease: false` for its own grunt and retains the `SoundData*`; returning null would change bookkeeping it depends on, and the pool slot is spent either way. ⚠ **That `volume: 0f` fully silences is unverified in game** — the fallback lever is `SoundVolumeCategory.NoPlay` (= 5).

The blunter Penumbra alternative — redirecting the `sound/voice/Vo_Battle/Vo_Battle_PC_{race}_{Ma|Fe}_{lang}.scd` paths to a silent SCD — was **not** built, and is worse than it looks: a redirect is resolved when the resource loads and is then cached, so it cannot be switched per cast. It could only ever have implemented the blanket mode.
...
> **✅ CLOSED 2026-08-18: all five criteria met in game** — (a) on 2026-08-17 via the
> spike, (b)–(e) via the Sound pack tab's checklist, which also confirmed the `speed`
> argument works on our containers. Results in `docs/native-spike.md`.
...
| **M9** | *v2: other players* | Flip the audience buckets on, add crowd scaling + token bucket, whitelist/friend/party UI, optional cast-start trigger. **Done 2026-08-24 except the cast-start trigger, plus two things the plan did not anticipate: per-profile targets, and widening the pack builder's warm scope from one job to the audience's jobs.** | **5 d** |
...
3. ~~Grunt suppression~~ — **DECIDED 2026-08-25: in v1, and with a blanket mode alongside it.** Both off by default. The second `SoundManager.PlaySound` hook is `Audio/GruntSuppressor`; see §5.6 and M10. What is left is in-game verification, not a decision.
