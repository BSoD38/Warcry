...
| M7 | Native sink | 🟡 **built, unverified in game (2026-08-17).** `ScdForge` + `NativeVoiceSink` + `CompositeVoiceSink` are wired into the real mapping pipeline behind `IVoiceSink`, gated on `PreferNativeSink` (**off by default**). Design points: `VoiceRequest` now carries a **provider factory and a variant key** rather than a live provider, because the forge has to *drain* a provider to encode it and a fallback would otherwise get an exhausted reader; encoding runs **off the game thread** (measured ~10 ms for a 5 s clip — over half a frame) with Penumbra registration pumped back on the main thread; the first play of each clip is spent warming the resource and reported as a refusal, so the composite serves that one line from NAudio. Native gain deliberately skips the game-slider scaling the managed sink applies by hand, since the engine applies its own bus volume downstream. Still gated on §6 (b)–(e), and the `SoundDataRefCount` leak is unaddressed. |
